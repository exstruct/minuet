if Code.ensure_compiled?(XmlBuilder) do
  defprotocol Minuet.Format.XML do
    def enter(type, vars)
    def exit(type, vars)
  end

  defimpl Minuet.Format.XML, for: Minuet.Type.Root do
  ...
  defimpl Minuet.Format.XML, for: Minuet.Type.Enumerable do
    def enter(%{line: line}, vars) do
      %{buffer: [buffer | _]} =
        vars =
        vars
  ...
  defimpl Minuet.Format.XML, for: Minuet.Type.Enumerable.Item do
    def enter(%{line: line}, vars) do
      %{buffer: [buffer | _]} =
        vars =
        vars
  ...
  defimpl Minuet.Format.XML, for: Minuet.Type.Element do
    def enter(_, vars) do
      {nil, vars}
    end
  ...
  defimpl Minuet.Format.XML, for: Minuet.Type.Element.Open do
    def enter(%{children: [], line: line}, vars) do
  ...
  defimpl Minuet.Format.XML, for: Minuet.Type.Element.Close do
    def enter(_, vars) do
      {nil, vars}
    end

    def exit(%{children: []}, vars) do
      {nil, vars}
    end
  ...
  defimpl Minuet.Format.XML, for: Minuet.Type.Constant do
    def enter(%{expression: expression, line: line}, vars) do
      %{buffer: [buffer | _]} = vars

      {quote line: line do
         unquote(buffer) = [
           unquote(XmlBuilder.generate(expression, format: :none))
           | unquote(buffer)
         ]
       end, vars}
    end
  ...
  defimpl Minuet.Format.XML, for: Minuet.Type.Value do
    def enter(%{expression: expression, line: line}, vars) do
      %{buffer: [buffer | _]} = vars

      {quote line: line do
         unquote(buffer) = [
           XmlBuilder.generate(unquote(expression), format: :none)
           | unquote(buffer)
         ]
       end, vars}
    end
